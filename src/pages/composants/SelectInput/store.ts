...
import headerWithLeftSidebarSelectInputStoreCustom from './store.custom'
import { $translate } from '~/modules/i18n'

const storeName = 'headerWithLeftSidebarSelectInput'
const customImplement: CustomStoreImplementationOptions = headerWithLeftSidebarSelectInputStoreCustom.options
const aliveStoreIds = initAliveStoreIds()

function headerWithLeftSidebarSelectInputFactory (storeId?: Nullable<StoreIdentifier>) {
  return defineStore(storeId == null ? storeName : `${storeName}/${storeId}`, {
    state: () => {
      return {
        ...
        // Page info,
        $title: $translate('select-input.title', 'Select Input'),
        $description: $translate('select-input.description', ''),
        $publishedTime: new Date('2023-06-08T09:02:02.207Z'),
        $modifiedTime: new Date('2023-07-26T14:11:38.791Z'),
        // Constants,
        $constants: {
          choixMultipleAutoCompleteItems: [
            'apple',
            'orange',
            'banana',
            'sdfsfds'
        ...
        $inputs: {
          ...
          choixMultipleAutoComplete: undefined as Nullable<Array<any>>,
          values: [
            'Apple',
            'Orange',
            'Banana'
    ...
    getters: {
      ...
      /* @ts-ignore: to allow override in your custom file */
      choixMultipleAutoComplete (): WritableComputedRef<Nullable<string> | Nullable<Array<any>>> {
        return computed({
          get: () => this.values.value,
          set: (value) => {
            this.values.value = value
    ...
    actions: {
      ...
      async titreTextClick () {
        await navigateTo({ hash: '#utilisation--p24' })
      },
      /* @ts-ignore: to allow override in your custom file */
      async titre1Click () {
        await navigateTo({ hash: '#api--p24' })
      },
      /* @ts-ignore: to allow override in your custom file */
      async titre2Click () {
        await navigateTo({ hash: '#example--p24' })
      },
      /* @ts-ignore: to allow override in your custom file */
      async sousTitreClick () {
        await navigateTo({ hash: '#props--p24' })
      },
      /* @ts-ignore: to allow override in your custom file */
      async sousTitreTextClick () {
        await navigateTo({ hash: '#icons--p24' })
      },
      /* @ts-ignore: to allow override in your custom file */
      async sousTitre1Click () {
        await navigateTo({ hash: '#multiple--p24' })
      },
      /* @ts-ignore: to allow override in your custom file */
...
function getStoreInstance (storeId?: Nullable<StoreIdentifier>) {
  return headerWithLeftSidebarSelectInputFactory(storeId)()
}

export const headerWithLeftSidebarSelectInputRaw = headerWithLeftSidebarSelectInputFactory()
export const headerWithLeftSidebarSelectInput = () => headerWithLeftSidebarSelectInputRaw() as MergeCustomStore<typeof headerWithLeftSidebarSelectInputStoreCustom.instance>
